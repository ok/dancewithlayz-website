"use client"

import Image from "next/legacy/image"
import { useState, useEffect } from "react"
import { GeistMono } from "geist/font/mono"
import { YTCard } from "@/components/wrapper/yt-card"
import { SCTrackRow, SCCompactPlayer, SCPlaylistPlayer } from "@/components/wrapper/sc-player"
import { displayFont } from "@/lib/fonts"

type TextBlocks = {
  artistDescription: string;
  artistDescriptionShort: string;

}

const textBlocks = {
  artistDescription:
    "After countless nights buried in the studio chasing the perfect groove, Lay'z decided one thing " +
    "wasn't up for debate: this music was going to move people. Nothing less than that would do. " +
    "To bring every feeling to the surface, Lay'z teams up with singers on select tracks while staying " +
    "hands-on with every beat, bassline, and drop. The mission is simple — turn the world into a " +
    "little more of a dance floor, one House, Dance, and EDM record at a time.",
  artistDescriptionShort:
    "After countless nights buried in the studio chasing the perfect groove, Lay'z decided one thing wasn't up for debate: this music was going to move people...",
}

const albumPlaylistUrl = "https://soundcloud.com/dancewithlayz/sets/human-in-a-loop";

type Video = {
  id: string;
  title: string;
  album: string;
};

// Define an array of YouTube video configurations
const videos: Video[] = [
  {
    id: "mOdXes5D904",
    title: "DJ'Z Heat",
    album: "n/a",
  },
  {
    id: "OWTMsEaQ3Q4",
    title: "Echo",
    album: "me are Lay\'z",
  },
];

// Featured Songs are now streamed straight from SoundCloud.
type FeaturedSong = {
  url: string;
  title: string;
};

const featuredSongs: FeaturedSong[] = [
  { url: "https://soundcloud.com/dancewithlayz/dj-z-heat", title: "DJ'Z Heat" },
  { url: "https://soundcloud.com/dancewithlayz/handel-it", title: "Handle it" },
  { url: "https://soundcloud.com/dancewithlayz/take-it-slow-wav", title: "Take it Slow" },
];

const introTrackUrl = "https://soundcloud.com/dancewithlayz/human-in-a-loop-intro";

export default function ArtistPage() {
  const [currentSection, setCurrentSection] = useState("Home")
  const [isExpanded, setIsExpanded] = useState(false);

  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          setCurrentSection(entry.target.id.charAt(0).toUpperCase() + entry.target.id.slice(1))
        }
      })
    }, { threshold: 0.5 })

    const sections = document.querySelectorAll("section[id]")
    sections.forEach((section) => observer.observe(section))

    return () => sections.forEach((section) => observer.unobserve(section))
  }, [])

  return (
    (<div className="min-h-screen flex flex-col relative">
      <div className="relative z-10 flex flex-col min-h-screen">
        <main className="flex-grow">
          {/* Hero */}
          <div className="container mx-auto px-4 pt-6 mb-8">
            <div
              className="hero-viewport relative w-full h-[70vh] overflow-hidden"
              style={{ borderRadius: "clamp(28px, 9vw, 140px) clamp(28px, 9vw, 140px) 0.5rem 0.5rem" }}
            >
              <div className="absolute inset-0 bg-background" />
              <Image
                src="/layz-logo.png"
                alt="Artist Name"
                layout="fill"
                objectFit="contain"
                priority
              />
              <div className="starfield" aria-hidden="true" />
              <div className="glass-glint" aria-hidden="true" />
              <div
                className="absolute inset-0"
                style={{
                  background: "linear-gradient(180deg, rgba(11,10,18,0.15) 0%, rgba(11,10,18,0.35) 55%, rgba(11,10,18,0.9) 100%)"
                }}
              />
              <div className="absolute inset-0 flex items-end pb-10">
                <div className="px-5 md:px-12">
                  <div className={`${GeistMono.className} eyebrow mb-2`}>House / Dance / EDM</div>
                  <h2 className={`${displayFont.className} text-white text-5xl md:text-7xl uppercase tracking-wide mb-3`}>
                    Lay&apos;z
                  </h2>
                  <div className={`${GeistMono.className} flex items-center gap-3`}>
                    <SCCompactPlayer url={introTrackUrl} label="Intro" />
                    <span className="text-xs uppercase tracking-[0.14em] text-white/70">
                      Human in the Loop &mdash; press play, no autopilot involved
                    </span>
                  </div>
                </div>
              </div>
            </div>

            {/* HUD strip: bio readout riveted flush under the viewport */}
            <div className={`${GeistMono.className} border border-t-0 hairline text-xs bg-background/70 backdrop-blur-md`}>
              <div className="px-5 md:px-12 py-5 md:py-6">
                <div className="uppercase tracking-[0.2em] text-[0.6rem] text-[hsl(var(--acid))] mb-2">About</div>
                <div className="max-w-2xl normal-case tracking-normal font-sans text-sm md:text-base text-foreground/85">
                  <p className="md:block hidden">
                    {textBlocks.artistDescription}
                  </p>
                  <div className="block md:hidden">
                    {isExpanded ? (
                      <p>
                        {textBlocks.artistDescription}
                        <button
                          onClick={() => setIsExpanded(false)}
                          className="text-[hsl(var(--acid))] hover:text-[hsl(var(--acid-dim))] ml-1"
                        >
                          Show less
                        </button>
                      </p>
                    ) : (
                      <p>
                        {textBlocks.artistDescriptionShort}
                        <button
                          onClick={() => setIsExpanded(true)}
                          className="text-[hsl(var(--acid))] hover:text-[hsl(var(--acid-dim))] ml-1"
                        >
                          Read more
                        </button>
                      </p>
                    )}
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div className="container mx-auto px-4">
            {/* Featured Song Section */}
            <section id="songs" className="mb-16 pt-16 -mt-16">
              <div className="flex items-baseline justify-between gap-4 mb-6 border-b hairline pb-4">
                <div>
                  <div className={`${GeistMono.className} eyebrow`}>Featured</div>
                  <h2 className="text-2xl md:text-3xl font-extrabold uppercase tracking-wide mt-1">Songs</h2>
                </div>
                <div className={`${GeistMono.className} text-xs text-muted-foreground`}>
                  {String(featuredSongs.length).padStart(2, "0")} tracks
                </div>
              </div>
              <div>
                {featuredSongs.map((song, i) => (
                  <SCTrackRow
                    key={song.url}
                    index={i + 1}
                    url={song.url}
                    fallbackTitle={song.title}
                  />
                ))}
              </div>
            </section>

            {/* Albums Section */}
            <section id="albums" className="mb-16 pt-16 -mt-16">
              <div className="flex items-baseline justify-between gap-4 mb-6 border-b hairline pb-4">
                <div>
                  <div className={`${GeistMono.className} eyebrow`}>Discography</div>
                  <h2 className="text-2xl md:text-3xl font-extrabold uppercase tracking-wide mt-1">Albums</h2>
                </div>
              </div>
              <SCPlaylistPlayer url={albumPlaylistUrl} />
            </section>

            {/* YouTube Videos Section */}
            <section id="videos" className="mb-16 pt-16 -mt-16">
              <div className="flex items-baseline justify-between gap-4 mb-6 border-b hairline pb-4">
                <div>
                  <div className={`${GeistMono.className} eyebrow`}>Watch</div>
                  <h2 className="text-2xl md:text-3xl font-extrabold uppercase tracking-wide mt-1">Videos</h2>
                </div>
                <div className={`${GeistMono.className} text-xs text-muted-foreground`}>
                  {String(videos.length).padStart(2, "0")} uploads
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {videos.map((video) => (
                  <YTCard key={video.id} id={video.id} title={video.title} album={video.album} />
                ))}
              </div>
            </section>
          </div>
        </main>
      </div>
    </div>)
  );
}
